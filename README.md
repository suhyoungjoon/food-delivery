...
각 어그리게잇중심으로 엑터 및 이벤트 연결
    ...
    - 도메인 서열 분리 
        - Core Domain:  구매 : 없어서는 안될 핵심 서비스이며, 연견 Up-time SLA 수준을 99.999% 목표, 배포주기는 app 의 경우 1주일 1회 미만, store 의 경우 1개월 1회 미만  # (구매에 자체 영화정보를 가지고 있는것으로 전제함)
        
        - Supporting Domain: 영화 : 경쟁력을 내기위한 서비스이며, SLA 수준은 연간 60% 이상 uptime 목표, 배포주기는 각 팀의 자율이나 표준 스프린트 주기가 1주일 이므로 1주일 1회 이상을 기준으로 함. # (현재 설계에서 구매시 영화정보는 자체적으로 가지고 있는걸로 전제하여 영화정보는 핵심이 아님)
